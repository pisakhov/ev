// Last update: 21Sep2023
// Version 0.5.3
// Added API for solver
...
class EconVision {
	constructor(debug = false) {
		...
		let parentItems = [{
			...
			"name": "setBounds",
			"children": [{"type": "description", "value": "Setting value for top and right will be Quadrant2. the values would be the desmos parameters and not idDivs. Tolerance and mirror-tolerance would be an extra gap. The value would be percentage (eg. 1.1 is 110%)"},{
				"type": "latex",
				"label": "top",
				"id": "setBounds-top",
				"value": "B_{t}"
			...
			},
			{
				"type": "number",
				"label": "tolerance",
				"id": "setBounds-tolerance",
				"value": "1.1"
			},
			{
				"type": "number",
				"label": "mtolerance",
				"id": "setBounds-mirror-tolerance",
				"value": "1.2"
			}, {
				"type": "text",
				"label": "listGraphs",
			...
			},
			{
				"type": "checkbox",
				"label": "defaultState",
				"id": "addSwitchInput-defaultState",
				"value": false
			}, {
				"type": "text",
				"label": "idDivs",
		...
		},
		{
			...
			"children": [{"type": "description","value": "In NewfunEqn, use iddivs in the parenthesis separated by commas"},{
				"type": "disabled",
				"label": "calc",
		...
		{
			"name": "addExpression",
			"displayName": "addExpression > simpleMarshalian (return List[x,y])",
			"children": [{"type": "description","value": "Based on NewfuncEqn, this will best bundle, supports min utilitiy as well to preset \\min(x,3y)"},{
				"type": "disabled",
				"label": "calc",
				"id": "simpleMarshalian-calc",
				"value": "simpleMarshalian"
			}, {
				"type": "text",
				"label": "idDiv",
				"id": "simpleMarshalian-idDiv",
				"value": "NewID"
			}, {
				"type": "text",
				"label": "parentIdDiv",
				"id": "simpleMarshalian-parentIdDiv",
				"value": 'UtilityFunction'
			},
			{
				"type": "text",
				"label": "constraint",
				"id": "simpleMarshalian-constraint",
				"value": 'totalBudgetLine'
			}, 
			{
				"type": "latex",
				"label": "NewfunEqu",
				"id": "simpleMarshalian-NewfunEqu",
				"value": "\\mu"
			}, {
				"type": "text",
				"label": "listGraphs",
				"id": "simpleMarshalian-listGraphs",
				"value": "[0]"
			}]
		},
		{
			...
			"children": [{"type": "description","value": "Based on NewfuncEqn, this will generate 3 additional iddivs, e.g. M_x, M_y. and M_lambda (value of the lamda)"},{
				"type": "disabled",
				"label": "calc",
				"id": "simpleLag-calc",
				"value": "simpleLag"
		...
		{
			"name": "addDynamicExp > simpleDrawLabel",
			"children": [{
				"type": "text",
				"label": "idDiv",
		...
		},
		{
			...
			"name": "addDynamicExp",
			"displayName": "addDynamicExp > simpleMarshalian (return List[x,y])",
	...
	addSwitchInput(options) {
		...
		let defaultState = (("defaultState" in options) ? options["defaultState"] : false);
		...
		<label class="' + idDiv + ' relative flex flex-col items-start cursor-pointer ml-2">\
		  <input type="checkbox" value="" id="' + idDiv + '" data-function="triggerCheck" class="sr-only peer" '+((defaultState)?"checked":"")+'>\
	...
	addExpression(options) {
		...
		} else if (calc === "simpleMarshalian") {
			addRefreshButton();
			runCalcEvent(simpleMarshalian);
			updateOnEvent(simpleMarshalian);
		...
		function simpleMarshalian(expression = "", subScript = "") {
			...
			if (mainFunc.includes('min(')) {
				// Handle min/max functions using kink point method
				...
				nerdamer.setFunction('U', ['x', 'y'], mainFunc);//5x+2sqrt(y)
				nerdamer.setFunction('B', ['x', 'y'], constraintFunc);//constraint -14+10*x+y
		
				const MUx = nerdamer.diff('U(x, y)', 'x').text();//5
				const MUy = nerdamer.diff('U(x, y)', 'y').text();//1/sqrt(y)
				const dBdx = nerdamer.diff('B(x, y)', 'x').text();//10
				const dBdy = nerdamer.diff('B(x, y)', 'y').text();//1
		
				const priceRatio = nerdamer(`${dBdx}/(${dBdy})`).text();// 10
				const MRS = nerdamer(`${MUx}/(${MUy})`).text();//// 5/(1/sqrt(y))=5*sqrt(y)
		...
		function simpleLag(expression = "", subScript = "") {
			...
			try {
				...
				console.log("Nerdamer Error:", error);
			
				// Making an API call if nerdamer fails
	...
	addDynamicExp(options) {
		...
		if (calc === "simpleDraw") {
			...
			addRefreshButton();
			runDynamicEvent(simpleMarshalian);
			updateOnEvent(simpleMarshalian);
		} else if (calc === "simpleLag"){
			addRefreshButton();
	...
	setInstructions(options) {
		function addPagination(numPage) {
			...
			var footul = '</ul></div>';
			var addReportBtn = '<button id="openModal" class="hidden md:block bg-red-500 text-white font-bold text-xs py-1 px-2 rounded group inline-flex items-center">\
								...
								';
			var modal = '<div id="modal" class="hidden left-0 top-0 fixed inset-0 flex items-center justify-center z-50">\
			<div class="bg-white rounded-lg w-full max-w-md h-[700px] p-6">\
				<div class="flex justify-between items-center">\
					<button id="closeModal" class="text-gray-600 hover:text-gray-800">&times; Close</button>\
				</div>\
				<iframe src="https://docs.google.com/forms/d/e/1FAIpQLSdA_UK6abG40jqzrj3_uc_GBXhC5gBLFWL4mq0kdUJnsBBgJw/viewform?embedded=true" class="w-full h-full" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe>\
			</div>\
		</div>\
		<script>\
			const openModalButton = document.getElementById("openModal");\
			const closeModalButton = document.getElementById("closeModal");\
			const currentPageForm = document.getElementById("currentPageForm");\
			const title = document.getElementById("title");\
			const description = document.getElementById("description");\
			const modal = document.getElementById("modal");\
			openModalButton.addEventListener("click", () => {\
				modal.classList.remove("hidden");\
				currentPageForm.value = window.location.href;\
				title.value = "";\
				description.value = "";\
			});\
